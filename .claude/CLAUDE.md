...
# Run SonarQube analysis (requires -Dall or -Dlint active)
mvn clean install -Dall -Dsonarqube
```
...
Python linting and tests (runs inside Docker in CI, but can be run locally):

```bash
ruff check python/ --config ci/python/ruff.toml --fix
pytest -p no:cacheprovider -v python/tests/
```
...
- **`CLOUDFLARE_CHECK`** — Uses the Python `undetected-chromedriver` service (Flask app on port 5000) to bypass
  Cloudflare bot detection. Extensions cannot be configured for this driver type.
...
`python/selenium_manager/` is a Flask server that wraps `undetected-chromedriver`. The Java side starts this process and
communicates with it over HTTP when executing `CLOUDFLARE_CHECK` trackers. `PythonWebDriverFactory` handles the
lifecycle; `AttachedRemoteWebDriver` connects to the running session.

### Redaction

`AbstractTrackerHandler.redactElements()` finds elements by the selectors returned from `ipAddressElements()`,
`emailElements()`, `passkeyElements()`, and `sensitiveElements()`, then applies the configured `RedactionType` (`TEXT`
replacement or `OVERLAY` box). Redaction happens after profile page load, before the screenshot.
