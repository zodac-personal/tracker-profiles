...
def test_open_and_close_session_no_extensions(mock_chrome: MagicMock, client: FlaskClient, tmp_path: Path) -> None:
...
@patch("selenium_manager.routes.uc.Chrome")
def test_open_and_close_session_two_extensions(mock_chrome: MagicMock, client: FlaskClient, tmp_path: Path) -> None:
    mock_driver = MagicMock()
    mock_driver.session_id = "test-session"
    mock_driver.service.service_url = "http://127.0.0.1:12345"
    mock_chrome.return_value = mock_driver
    ...
    res_open = client.post("/open", json={
        ...
        "enable_translation": True,
        "extension_paths": [
            "/app/ublock_origin_lite.crx",
            "/app/some_other_extension.crx",
        ],
    })
